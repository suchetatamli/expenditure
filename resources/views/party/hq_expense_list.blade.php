...
<table class='table table-striped table-bordered'>
  ...
  @if (sizeof($result) >0)
    @foreach($result as $row)
      <tr>
        ...
        <td style="text-align: center;">
          ...
          @elseif($row->tran_method == 'draft') {{'Draft'}}
          @elseif($row->tran_method == 'online_payment') {{'Online Payment'}}
          @else {{'Cash'}}
         @endif</td>
        <!-- <td>{{ $row->receipt_no}}</td> -->
        <td style="text-align: right;">{{formatCurrency($row->tran_amount)}}</td>
        <?php $amt = getOutstandingAmountSchedules($row->tag_schedule,$row->id); ?>
    ...
    <tr>
        ...
        <td style="text-align: right; font-weight: bold;">{{formatCurrency($totAmtOut)}}</td>
        <td style="text-align: center;">
        </td>
