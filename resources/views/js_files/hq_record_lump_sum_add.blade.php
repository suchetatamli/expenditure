<script>
   ...
   $("#payment_mode").on('change',function(){
      ...
      if (payment_mode == 'Cheque' || payment_mode == 'DD' || payment_mode == 'PO') {
         ...
         expense_bank_html += '<select class="form-control" name="expenses_bank" required><option value="">Select Bank</option>{{ getBankList() }}</select>';
   ...
   $(document).on('submit','form',function(){ 
      if($('input[name=amount]').val() > 10000 && $('#payment_mode').val() == 'Cash'){
         swal({
            title: "Information",
            text: "All lump sum amount above Rs. 10,000/- are to be incurred through your bank account",
