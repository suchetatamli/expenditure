...
<table class='table table-striped table-bordered'>
  ...
  <tbody>
    ...
    @php
    if(gettype(trim($row->tran_purpose))   == 'integer')
      $sch = $row->expense_type;
    elseif(strlen(trim($row->tran_purpose)) <= 2)
      $sch = $row->expense_type;
    else
      $sch = $row->tran_purpose;
    @endphp
      <tr>
        <td>{{indianDateFormat($row->tran_date)}}</td>
        <td>{{formatCurrency($row->tran_amount)}} for {{$sch}}</td>
        <td>
